...
export default function EventSearch({ category: selectedCategory }) {
  ...
  useEffect(() => {
    ...
    const filtered = events.filter((event) => {
      ...
      const dateMatch = new Date(event.date).toLocaleDateString().toLowerCase().includes(term);
      ...
      return (
        (titleMatch || locationMatch || categoryMatch || dateMatch) && categoryFilter && dateFilter
      );
    });
            ...
            >
              <Swiper
                modules={[
                  ...
                  Pagination,
                  Autoplay,
                  ...(enable3DEffect ? [EffectCoverflow] : []),
                ]}
                slidesPerView="auto"
